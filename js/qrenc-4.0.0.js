...
function QR(obj, text, w, h, ecc)
{
	...
	var qf = this.genframe(text);

	var truewidth = w;
	if( h < w )
		truewidth = h;
	var border = Math.floor((truewidth / this.width) / 2); // we pad the image by the amount of a single px on each side
	var px = Math.floor((truewidth - border * 2) / this.width);
	var availwidth = px * (this.width);
	var offset = (truewidth - availwidth) / 2;
	ctx.clearRect(0,0,w,h);
	ctx.fillStyle = '#fff';
	ctx.fillRect(0,0,truewidth,truewidth);
	ctx.fillStyle = '#000';
	ctx.beginPath();
...
QR.prototype.setmask = function(x, y)
{
    var bt;
    if (x > y) {
        bt = x;
...
QR.prototype.putalign = function(x, y)
{
    var j;
...
QR.prototype.appendrs = function(data, dlen, ecbuf, eclen)
{
    var i, j, fb;
    ...
    for (i = 0; i < dlen; i++) {
        fb = QR.glog[this.strinbuf[data + i] ^ this.strinbuf[ecbuf]];
        if (fb != 255)     /* fb term is non-zero */
            for (j = 1; j < eclen; j++)
                this.strinbuf[ecbuf + j - 1] = this.strinbuf[ecbuf + j] ^ QR.gexp[this.modnn(fb + this.genpoly[eclen - j])];
        else
            for( j = ecbuf ; j < ecbuf + eclen; j++ )
                this.strinbuf[j] = this.strinbuf[j + 1];
        this.strinbuf[ ecbuf + eclen - 1] = fb == 255 ? 0 : QR.gexp[this.modnn(fb + this.genpoly[0])];
    }
};
...
QR.prototype.ismasked = function(x, y)
{
    var bt;
    if (x > y) {
        bt = x;
...
QR.prototype.applymask = function(m)
{
    var x, y, r3x, r3y;
...
QR.prototype.badruns = function(length)
{
    var i;
    var runsbad = 0;
    for (i = 0; i <= length; i++)
...
QR.prototype.badcheck = function()
{
    var x, y, h, b, b1;
    var thisbad = 0;
    var bw = 0;
...
QR.prototype.genframe = function(instring)
{
    var x, y, k, t, v, i, j, m;
...
// find the smallest version that fits the string
    ...
    do {
        ...
        this.neccblk1 = QR.eccblocks[k++];
        this.neccblk2 = QR.eccblocks[k++];
        this.datablkw = QR.eccblocks[k++];
        this.eccblkwid = QR.eccblocks[k];
...
// calculate generator polynomial
    ...
    for (i = 0; i < this.eccblkwid; i++) {
        ...
        for (j = i; j > 0; j--)
            ...
            ? this.genpoly[j - 1] ^ QR.gexp[this.modnn(QR.glog[this.genpoly[j]] + i)] : this.genpoly[j - 1];
        this.genpoly[0] = QR.gexp[this.modnn(QR.glog[this.genpoly[0]] + i)];
    }
    for (i = 0; i <= this.eccblkwid; i++)
        this.genpoly[i] = QR.glog[this.genpoly[i]]; // use logs for this.genpoly[] to save calc step
...
// add in final mask/this.ecclevel bytes
    y = QR.fmtword[t + ((this.ecclevel - 1) << 3)];
    // low byte
    for (k = 0; k < 8; k++, y >>= 1)
        if (y & 1) {
